...
Pin = 123456  # ubah pin kalau mau

# pastikan dijalankan sebagai root
if os.geteuid() != 0:
    print("jalankan script ini sebagai root (sudo su / login root). keluar.")
    sys.exit(1)
...
class CRDRootKDE:
    ...
    @staticmethod
    def install_kde():
        print("-> menginstall KDE Plasma (standard, tanpa audio config)...")
        os.system("apt update -y && apt install --assume-yes kde-standard sddm dbus-x11 dbus-user-session")
        print("-> KDE Plasma terpasang")
    ...
    def configure_crd_session():
        print("-> menulis /etc/chrome-remote-desktop-session (KDE root session)...")
        ...
        print("-> session KDE root ditulis")
    ...
    def finish():
        ...
        if CRD_SSH_Code:
            print("-> jalankan host setup (ikuti instruksi Google di terminal)...")
            os.system(f"{CRD_SSH_Code} --pin={Pin}")
        else:
            print("-> tidak ada CRD auth code, dilewati")

        print("\n✅ selesai. KDE Plasma siap diakses via Chrome Remote Desktop sebagai root.")
...
if __name__ == "__main__":
    if CRD_SSH_Code.strip() == "":
        ...
        sys.exit(1)
    if len(str(Pin)) < 6:
        print("pin minimal 6 digit. ubah variabel Pin.")
        sys.exit(1)
    CRDRootKDE()
